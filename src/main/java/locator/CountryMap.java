...
public class CountryMap {
    ...
    private Map<String , Field> fields = new HashMap<String, Field>(1000);
    private String fileName = "CityCountryLatLon.csv";

    public static synchronized CountryMap getInstance(String iso, Long minPopulation) {
        
        if (instance == null || !ISO.equals(iso)){ 
            instance = new CountryMap(iso, minPopulation); 
        }
        return instance;
    }

    public CountryMap(String iso, Long minPopulation){
        this.ISO = iso;
        System.out.println("Recomputing "+iso+" map!");
    try{
                ...
                String line = "";
                try {
                        fis = new FileInputStream(ConfigPath.getConfigPath() + "/"+ fileName);
                        isr = new InputStreamReader(fis);
                        buf = new BufferedReader(isr);
                        while ( (line = buf.readLine()) != null ) {
                            
                            Point c = new Point(line);
                            if(c.getIsoCountry().equals(iso)){
                                if(minPopulation!=null && c.getPopulation() < minPopulation)
                                    continue;
    ...
    public String getCountry(float lat, float lon){
        ...
        if(fields.containsKey(field)){
            ...
            //System.out.println("NO US FIELD :: LAT , LON = ["+(lat-90)+" ,"+( lon-180)+" ]");
        }
...
//        }
    ...
    public Point(String s){
        ...
        cityName = line[0];
        // non identifico country perchè è di default quella richiesta.
        cityCountry = line[0];
        isoCountry = line[2];
